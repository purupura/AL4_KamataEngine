...
float EaseInSine(float a) { return 1 - cosf((a * (float)M_PI) / 2); }

float EaseOutSine(float x) { return sinf((x * (float)M_PI) / 2); }

float EaseInOutSine(float x) { return -cosf((x * (float)M_PI) - 1) / 2; }
...
float EaseOutCubic(float x) { return 1 - powf(1 - x, 3); }
...
float EaseInQuart(float x) { return powf(x, 4); }

float EaseOutQuart(float x) { return 1 - powf(1 - x, 4); }
...
float EaseInQuint(float x) { return powf(x, 5); }

float EaseOutQuint(float x) { return 1 - powf(1 - x, 5); }
...
float EaseInCirc(float x) { return 1.0f - sqrtf(1 - powf(x, 2.0f)); }
...
Vector3 LerpVector(const Vector3& v1, const Vector3& v2, float t) { return Vector3(Lerp(v1.x, v2.x, t), Lerp(v1.y, v2.y, t), Lerp(v1.z, v2.z, t)); }